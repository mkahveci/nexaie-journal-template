# Nexus AI & Education (NExAIE) Article Template

Official LaTeX template for the **Nexus AI & Education (NExAIE)** journal. This template is designed for AI-native scholarly publishing with a focus on transparency, accessibility, and high-impact dissemination.

## 🚀 Key Features
*   **AI-Native Disclosure**: Integrated support for [PEDAL Archive](https://kahveci.pw/pedal) protocol citations.
*   **Accessibility First**: Built-in ADA/WCAG 2.1 compliance for figures (forced Alt-Text).
*   **Scholarly Infrastructure**: Multi-author/institution support via `authblk`.
*   **Flexible Layouts**: Toggles for `onecolumn`, `twocolumn`, `review` (line numbers), and `preprint` modes.
*   **Biber/BibLaTeX**: Modern citation management using APA 7th Edition.

## 📥 Getting Started
1.  **Journal Homepage**: [nexaie.kahveci.pw](https://nexaie.kahveci.pw/index.php/nexaie)
2.  **Overleaf (Recommended)**: [Official Template](https://www.overleaf.com/read/pqtdqgxkkzrm#72e041)
3.  **Local Use**:
    *   Clone this repository.
    *   Ensure you have a modern LaTeX distribution (TeX Live 2024+).
    *   Compile using `pdflatex` -> `biber` -> `pdflatex`.

## 📂 File Structure
*   `nexaie.cls`: The official journal class file.
*   `main.tex`: Sample submission file.
*   `author_guide.md`: Technical writing specifications.
*   `media/`: Journal branding and logos.
*   `sections/`: Modularized paper sections.

## ⚖️ License
This template is licensed under the **MIT License**. The content of the articles published using this template is typically licensed under **CC BY 4.0**.

## 📧 Support
For issues with the template, please [open an issue](https://github.com/mkahveci/nexaie-template/issues) or contact `editor@kahveci.pw`.